<script lang="ts">
	...
	const cossuiUrl = 'https://coss.com/ui';
</script>

<ParticleCardContainer class={className} {colSpan}>
	{#snippet children()}
		<div data-particle data-slot="preview" class="flex items-center justify-center p-4">
	...
	{#snippet footer()}
		...
		<div class="flex items-center gap-1.5">
			<CopyRegistry value="{cossuiUrl}/r/{particle.name}.json" variant="outline" />
			<Drawer position="right">
				<DrawerTrigger>
					{#snippet child({ props })}
						<Button {...props} class="text-sm" size="sm" variant="outline">View code</Button>
					{/snippet}
				</DrawerTrigger>
					...
					<div class="flex flex-1 flex-col overflow-hidden p-6">
						...
						<div class="flex h-full flex-1 flex-col overflow-hidden">
							<div class="flex items-center justify-between gap-2">
								<h2 class="mt-6 mb-4 font-semibold text-xl">Code</h2>
								<Button
									href="https://v0.dev/chat/api/open?url={encodeURIComponent(`${cossuiUrl}/r/${particle.name}.json`)}"
									variant="outline"
									target="_blank"
