	...
	<CPComponents.wrapper bind:wrapper {isOpen} {isDialog}>
		...
		<div class="h">
			<Slider
				...
				aria-label={getTexts().label.h}
				bind:value={() => hsv?.h ?? _hsv.h, updateLetter('h')}
			/>
		</div>
		{#if isAlpha}
			<div class="a" style:--alphaless-color={(hex ? hex : _hex).substring(0, 7)}>
				<Slider
					...
					step={0.01}
					orientation={sliderDirection}
					aria-label={getTexts().label.a}
					bind:value={() => hsv?.a ?? _hsv.a, updateLetter('a')}
				/>
			</div>
		{/if}
...
<style>
	...
	.horizontal .h,
	.horizontal .a {
		width: calc(var(--picker-width, 200px) - 12px);
	...
	/* restyle the shared ui/slider to render as a gradient track without a progress indicator */
	.h :global([data-slot='slider']),
	.a :global([data-slot='slider']) {
		width: 100%;
		height: 100%;
		min-width: 0;
		min-height: 0;
	}
	.h :global([data-slot='slider-track']),
	.a :global([data-slot='slider-track']) {
		width: 100%;
		height: 100%;
	}
	.h :global([data-slot='slider-track'])::before,
	.a :global([data-slot='slider-track'])::before {
		inset: 0;
		background: var(--track-background);
	}
	.h :global([data-slot='slider-indicator']),
	.a :global([data-slot='slider-indicator']) {
		display: none;
	}
	.h :global([data-slot='slider-thumb']),
	.a :global([data-slot='slider-thumb']) {
		width: calc(var(--slider-width, 10px) - 3px);
		height: calc(var(--slider-width, 10px) - 3px);
		background: white;
