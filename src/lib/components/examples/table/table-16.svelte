<script lang="ts">
	...
	function handleDragEnd(event: DragEndEvent) {
		...
		if (
			snapshottedActive &&
			snapshottedOver &&
			snapshottedActive.id !== snapshottedOver.id
		) {
			table.setColumnOrder((columnOrder) => {
				const oldIndex = columnOrder.indexOf(
					snapshottedActive.id as string,
				);
				const newIndex = columnOrder.indexOf(
					snapshottedOver.id as string,
				);
	...
	function createHeaderDragAttachment(header: Header<User, unknown>): {
		...
		const buttonAttachment: Attachment = (element) => {
			...
			const mouseDownCleanup = on(
				element,
				"mousedown",
				listeners.current.onmousedown,
			);
			const touchStartCleanup = on(
				element,
				"touchstart",
				listeners.current.ontouchstart,
			);
			const keyDownCleanup = on(
				element,
				"keydown",
				listeners.current.onkeydown,
			);
	...
	const dragAlongCellAttachment = (cell: Cell<User, unknown>): Attachment => {
		const { isDragging, isSorting, setNodeRef, transform, transition } =
			useSortable({
				id: cell.column.id,
			});
...
{#snippet DraggableTableHeader(header: Header<User, unknown>)}
	{@const { buttonAttachment, thAttachment } =
		createHeaderDragAttachment(header)}
	<TableHead
