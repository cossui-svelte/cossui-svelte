<script lang="ts">
  ...
  let animationState = $state<'entering' | 'entered' | 'exiting' | 'stacking'>('entering');
  ...
  let iconConfig = $derived(hasVariantIcon(variant) ? VARIANT_ICONS[variant] : undefined);
  ...
  const shiftFocusOnDismiss = () => {
    ...
    const siblings = Array.from(document.querySelectorAll<HTMLElement>('[data-toast-id]')).filter(
      (el) =>
        el !== toastRef && el.getAttribute('data-limited') !== 'true' && !el.hasAttribute('inert')
    );
  ...
  const handlePointerUp = (event: PointerEvent) => {
    ...
    if (axis && !swipeCancelled) {
      ...
      const meetsThreshold =
        Math.abs(distance) >= SWIPE_DISMISS_THRESHOLD || velocity > SWIPE_DISMISS_VELOCITY;
  ...
  let defaultCollapsedOffset = $derived(
    isTopPosition ? index * ANIMATION_CONFIG.STACK_OFFSET : -(index * ANIMATION_CONFIG.STACK_OFFSET)
  );
  ...
  let visibleScale = $derived(
    Math.max(ANIMATION_CONFIG.MIN_SCALE, 1 - visibleIndex * ANIMATION_CONFIG.SCALE_FACTOR)
  );
  ...
  let config = $derived(POSITION_CONFIGS[(position ?? 'bottom-center') as ToastPosition]);
  ...
  let transitionTimingFunction = $derived(
    animationState === 'exiting' ? ANIMATION_CONFIG.EASING_EXIT : ANIMATION_CONFIG.EASING_DEFAULT
  );
      ...
      <div class={cn(toastContentVariants({ variant }))}>
        ...
        <div class="p-4 pe-8">
          <div class="flex gap-3">
                ...
                {#if shouldRenderSpinner}
                  <span
                    class={cn(
                      ...
                      spinnerState === 'loading' ? 'vs-spinner--active' : 'vs-spinner--finish'
                    )}
            ...
            <div class="min-w-0">
              {#if title}
                <div id={titleId} class="mb-1 text-sm leading-none font-medium select-none">
                  {title}
                </div>
              {/if}
              {#if description}
