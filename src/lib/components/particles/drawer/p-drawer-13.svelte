        ...
        <nav class="-m-2 flex flex-col">
          ...
          >
            <span class="size-3.5 flex items-center justify-center">
              {#if shuffle}<svg viewBox="0 0 12 12" fill="currentColor" class="size-3" aria-hidden
                  ><path
                    d="M10.707 3.293a1 1 0 010 1.414l-6 6A1 1 0 013.293 9.293l6-6a1 1 0 011.414 0z"
                  /><path
                    d="M10.707 8.707a1 1 0 01-1.414 0l-6-6A1 1 0 014.707 1.293l6 6a1 1 0 010 1.414z"
                  /></svg
                >{/if}
            </span>
            Shuffle
          ...
          >
            <span class="size-3.5 flex items-center justify-center">
              {#if repeat}<svg viewBox="0 0 12 12" fill="currentColor" class="size-3" aria-hidden
                  ><path
                    d="M10.707 3.293a1 1 0 010 1.414l-6 6A1 1 0 013.293 9.293l6-6a1 1 0 011.414 0z"
                  /><path
                    d="M10.707 8.707a1 1 0 01-1.414 0l-6-6A1 1 0 014.707 1.293l6 6a1 1 0 010 1.414z"
              ...
              >
                <span class="size-3.5 flex items-center justify-center">
                  {#if sortBy === value}<svg
                      ...
                      fill="currentColor"
                      class="size-2"
                      aria-hidden><circle cx="4" cy="4" r="4" /></svg
                    >{/if}
                </span>
                {label}
              </button>
          ...
          <button
            type="button"
            class="flex min-h-9 w-full cursor-default select-none items-center justify-between gap-4 rounded-sm px-2 py-1 text-base text-foreground outline-none hover:bg-accent hover:text-accent-foreground sm:min-h-8 sm:text-sm"
            onclick={() => {
              autoSave = !autoSave;
            }}
          >
            Auto save
            <span
              class="relative ms-auto inline-flex h-[1.125rem] w-8 shrink-0 items-center rounded-full transition-colors {autoSave
                ? 'bg-primary'
                : 'bg-input'}"
            >
              <span
                class="pointer-events-none block size-3.5 rounded-full bg-background shadow-sm transition-transform {autoSave
          ...
          <DrawerNestedRoot>
            ...
            <DrawerPopup showBar>
              <DrawerPanel>
                <nav class="-m-2 flex flex-col">
                  <div class="flex flex-col">
                    <div class="px-2 py-1.5 font-medium text-muted-foreground text-xs">
                      Add to Playlist
                    </div>
                  </div>
