<script lang="ts">
  ...
  const pageRows = $derived(
    sortedFlights.slice(pageIndex * pageSize, pageIndex * pageSize + pageSize)
  );
  ...
  const isAllPageSelected = $derived(
    pageRows.length > 0 && pageRows.every((row) => rowSelection[row.id])
  );
        ...
        <TableRow data-state={rowSelection[flight.id] ? 'selected' : undefined}>
          ...
          <TableCell>
            ...
            >
              ...
              ></div>
              <div
                class={cn('text-muted-foreground', flight.status === 'Cancelled' && 'line-through')}
              >
                {flight.duration}
              </div>
              <div
          ...
          <TableCell>
            <Badge variant="outline">
              <span
                aria-hidden="true"
                class={cn('size-1.5 rounded-full', getStatusColor(flight.status))}
              ></span>
              {flight.status}
            </Badge>
          </TableCell>
