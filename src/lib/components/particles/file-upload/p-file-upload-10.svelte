...
<div class="flex flex-col gap-2">
  <!-- Drop area -->
  <div
    class="flex min-h-56 flex-col items-center not-data-[files]:justify-center rounded-xl border border-input border-dashed p-4 transition-colors has-[input:focus]:border-ring has-[input:focus]:ring-[3px] has-[input:focus]:ring-ring/50 data-[dragging=true]:bg-accent/50"
  ...
  >
    ...
    {#if fileUpload.files.length > 0}
      <div class="flex w-full flex-col gap-3">
        ...
        <div class="w-full space-y-2">
          ...
          {#if fileUpload.files.length < maxFiles}
            <Button class="mt-2 w-full" onclick={fileUpload.openFileDialog} variant="outline">
    ...
    {:else}
      <div class="flex flex-col items-center justify-center text-center">
        ...
        <p class="text-muted-foreground text-xs">Max {maxFiles} files &middot; Up to {maxSize}MB</p>
