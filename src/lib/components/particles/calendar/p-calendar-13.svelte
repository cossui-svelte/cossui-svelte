<script lang="ts">
  import { CalendarDate, type DateValue, DateFormatter, getLocalTimeZone, today } from "@internationalized/date";
  ...
  const monthFormatter = new DateFormatter("en-US", { month: "long" });
</script>
  ...
  {#snippet children({ months, weekdays })}
    <Calendar.Months>
      ...
      {#each months as month (month)}
        <Calendar.Month>
          <Calendar.Header>
            <div class="flex w-full flex-1 items-center justify-center gap-1.5 *:flex-1 *:text-center">
              <span>{monthFormatter.format(month.value.toDate(getLocalTimeZone()))}</span>
              <Combobox
                autoHighlight
                items={yearItems}
