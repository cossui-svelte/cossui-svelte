...
export class CropperState {
  imageSize = $state<ImageSize>({ width: 0, height: 0, naturalWidth: 0, naturalHeight: 0 })
  ...
  cropperSize = $derived.by<Size | null>(() => {
    ...
    return cropSize ?? helpers.getCropSize(this.imageSize.width, this.imageSize.height, this.#options.aspect())
  })
