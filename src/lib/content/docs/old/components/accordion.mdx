---
title: Accordion
description: A set of collapsible panels with headings and content.

links:
  doc: https://base-ui.com/react/components/accordion#api-reference
---

<script lang="ts">
  import ComponentPreview from '$lib/components/app/docs/component-preview.svelte';
  import ComponentSource from '$lib/components/app/docs/component-source.svelte';
  import CodeTabs from '$lib/components/app/docs/code-tabs.svelte';
  import Steps from '$lib/components/app/docs/steps.svelte';
  import Step from '$lib/components/app/docs/step.svelte';
  import { TabsList, TabsTab, TabsPanel } from '$lib/components/ui/tabs';
</script>

<ComponentPreview name="p-accordion-1" align="start" />

## Installation

<CodeTabs>

<TabsList>
  <TabsTab value="cli">CLI</TabsTab>
  <TabsTab value="manual">Manual</TabsTab>
</TabsList>

<TabsPanel value="cli">

```bash
npx shadcn@latest add @coss/accordion
```

</TabsPanel>

<TabsPanel value="manual">

<Steps>

<Step>Install the following dependencies:</Step>

```bash
npm install @base-ui/react
```

<Step>Copy and paste the following code into your project.</Step>

<ComponentSource name="accordion" title="components/ui/accordion.svelte" />

<Step>Update the import paths to match your project setup.</Step>

</Steps>

</TabsPanel>

</CodeTabs>

## Usage

```svelte
<script lang="ts">
  import { Accordion, AccordionItem, AccordionPanel, AccordionTrigger } from '$lib/components/ui/accordion';
</script>
```

```svelte
<Accordion>
  <AccordionItem value="item-1">
    <AccordionTrigger>Is it accessible?</AccordionTrigger>
    <AccordionPanel>
      Yes. It adheres to the WAI-ARIA design pattern.
    </AccordionPanel>
  </AccordionItem>
</Accordion>
```

## API Reference

### Accordion

Root component.

### AccordionItem

Individual accordion item container. Styled wrapper with bottom border styling.

### AccordionTrigger

Trigger button that toggles the accordion panel. Includes a built-in chevron icon that rotates when expanded.

### AccordionPanel

Collapsible content panel with animated height transitions.

## Examples

### Single Accordion

<ComponentPreview name="p-accordion-2" align="start" />

### Multiple Accordion

<ComponentPreview name="p-accordion-3" align="start" />

### Controlled Accordion

<ComponentPreview name="p-accordion-4" align="start" />
